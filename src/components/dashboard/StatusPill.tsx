...
const TONE: Record<PillTone, { dot: string; text: string; ring: string }> = {
  ok: { dot: 'bg-emerald-400', text: 'text-emerald-300', ring: 'ring-emerald-400/25' },
  warn: { dot: 'bg-accent', text: 'text-accent', ring: 'ring-accent/30' },
  err: { dot: 'bg-rose-500', text: 'text-rose-300', ring: 'ring-rose-500/30' },
  idle: { dot: 'bg-fg-muted', text: 'text-fg-muted', ring: 'ring-white/10' },
}
...
export function StatusPill({ tone, label, pulse = false, title }: StatusPillProps) {
  ...
  return (
    <span
      title={title}
      className={`inline-flex items-center gap-1.5 rounded-full bg-white/4 px-2.5 py-1 text-[11px] font-semibold ring-1 ${t.ring} ${t.text}`}
    >
